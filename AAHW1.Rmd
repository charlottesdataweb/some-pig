...
Now that R and GitHub have started a relationship, here's the actual code breakdown:

A. Step one is to set the seed for the sake of randomness!

```{r, set.seed(69)}
knitr::opts_chunk$set(cache.extra = knitr::rand_seed)
```

B. Next step is to ensure all the needed packages are installed and called into the library. This should include the 'haven' package in order to properly import the .sav file (SPSS file)
...
C. Now we're all ready to load the dataset into R. The 'some-pig' project should (fingers crossed) be connected to my GitHub repository to make this project, and any changes I make to it, available through GitHub as well as R. I'm loading the data and calling it 'hwdata'
...
str(hwdata)
```
E. Now I know we're dealing with 23 questions, all on a scale of 1-5, from 'Strongly agree' to 'Strongly disagree' - my first question/point of clarification after viewing this information is if I should conclude from this that Strongly agree = 1 (vs. Strongly disagree = 1). Based on the way it's written, it looks like 1 = Strongly agree, but I will double check. This will be useful information to help with further interpretations of any analyses.
Also, most of the items had a 1-5 numerical label, but a few said that the options were 1, 2, 3, 4, 5, and 9. However, I didn't see any 9s when I looked at the data,
I also know that there are 8 factor scores pertaining to 2 different analyses. These may be useful later on, but for now, I plan to focus on the first 23 items.
...
print(scalelabels)
```
This helped a ton! Now I know that 1 = Strongly agree (often times it's the inverse, so that's good to know). I also know that 9 corresponds with not answered/NA. At a cursory glance, I didn't see any '9' responses so it looks like maybe those were already taken care of. However, I will be on the lookout as I move forward and ensure any 9s are changed to NA.
...
1. Before performing an EFA, I will be evaluating the data for the following:
a. Outliers
b. Linearity
c. Correlation
d. Sampling

a. First, I will search for outliers by measuring the Mahalanobis distance. Mahalanobis distance is how far away the data point is from the overall distribution of the data. Are there any cases of extreme or unusual responses that may mess with EFA down the line?
```{r}
set.seed(52)
...
```
I got this error above that said the system is computationally singular. Then I remembered that the dataset had the survey responses, as well as the factor analysis scores. So I will build a new dataset with only the survey responses and rerun the Mahalanobis code with that to see if that will work.

To create a new dataframe, I'm going to 'borrow' the code we learned from the dplyr package to deselect the last eight columns so I have a new dataset that is only survey responses.
...
My output (97 FALSE) indicates 97 potential outliers, but I'll take a deeper look at them. THe code below creates a new dataframe with the Mahalanobis distance values as a new column. The '24' in the code below denotes that it will be the 24th column in the new dataset
```{r}
...
Now I can look only at the 97 FALSE folks to examine their responses more closely.
```{r}
mahal_out <- hwdataitems_mahal %>%
    filter(mahal > cutoff) %>%
    arrange(desc(mahal)) # sort mahal values from most to least
```
I definitely see some extreme responses, as well as some unusual patterns of responses in the potential outliers. Some respondents selected almost exclusively 1s and 5s, and their patterns of selection don't make a lot of sense. For instance, some have the same or similar responses to Q1 ('Statistics makes me cry') and to Q3 ('Standard deviations excite me') even though they should be more opposing views. Or some selected opposing responses to Q8 ('I have never been good at mathematics') and Q11 ('I did badly at mathematics in school') even though intuitively, those responses should align well with one another.
Because of this, it is probably best practice to remove the outliers - plus, it will give me experience in the outlier removal process! 
```{r}
#bye bye outliers!
...
Cool, our new data set, nomoreout, has 2474 observations instead of the 2571. Now that we have dealt with outliers, we can move on to the other assumption checks before pressing on with an EFA.


b. Next, I'm going to examine the linearity of the data. I will look at this a few different ways, starting with additivity. I did use ChatGPT to give me more insights into what additivity means (if we covered it in ORM2, it has left the building!)
Overall, it is the concept that the effects of the individual predictors are independent of each other, that the effects of the predictor variables are additive (simply the sum of their individual effects). In general, we want items in our data to be correlated with each other, but not too correlated (multicollinearity/redundancy become an issue then).
```{r}
#looking at linearity: additivity
...
correl
```
Good news, I don't see any 1s beyond those in the diagonal. There are a few .3 and .6 values, but nothing to be concerned about here.

To further look at my assumptions, including the assumption of linearity, I'm going to first set up a fake regression analysis 
...
Now my 'fake' regression is all set up.
Next up, I'll check for normality
```{r}
#normality check
hist(standardized)
```
We're looking approximately normally distributed

Next, I'll test for heteroscedasticity with the Breusch-Pagan Test. Heteroscedasticity (if I remember corretly for ORM) is unequal variance of the residuals. So if residuals were more dispersed at lower X-values but less dispersed at higher X-values, that would consistute heteroscedasticity. This is a violation of the assumption of equal variance, so we are hoping for homoscedasticity, or similar variance, within the model.
```{r}
#load lmtest library
library(lmtest)
...
Now we can look at linearity with a Q-Q plot. A Q-Q plot is a plot that looks at two different samples to see if they are likely from the same population. Here we have a theoretical sample and actual sample. We are hoping for something highly resembling a straight line to provide sufficient evidence of linearity.
```{r}
...
We've got a little bit of a curve at the high and low ends, but the line is roughly straight so we will move forward.

Up next, I'll look at homogeneity of variance. This is similar to the concept of homoscedasticity discussed above. Is there roughly equal variance of our residuals throughout?
What we hope to see is a random, patternless plot here.
```{r}
...
We are looking pretty random, but not perfect. There seems to be skewness going on with less disperssion below the 0 standardized line and greater dispersion above 0. However, there is not a fan shape or anything funky going on left to right, so we will move forward with what I'll call 'pretty good homogeneity'


Overall, there is presence of linearity here. Is it perfect? No. But it is likely sufficient enough to more forward with an EFA, so away we go.

c. Now I can look at correlation with Bartlett's test. Once more, we are hoping that the items in the data are correlated with each other. If they are not correlated enough, the below results will be non-significant (which would indicate a problem...our items would not be related enough for EFA results to make sense). It's worth noting that we have a very large sample size (2474 respondents) so very likely our results will be significant even if the inter-item correlations are smaller.
```{r}
##correlation adequacy Bartlett's test
cortest.bartlett(correl, n = nrow(nomoreout))
```

We have statistical significance, p > .05 (yay), so correlations are sufficient for EFA. I also reexamined the correlation matrix from the tests for additivity and linearity, so I can be pretty confident that there is not the presence of dangeriously high collinearity among my items too. We are ready for the final step to ensure the data is well-fitted for an EFA.


d. Next, we can run the KMO Measure of Sampling Adequacy Test. This helps determine if there is enough of a relationship among the variables in the data to run (and be able to trust the results of) a factor analysis. It does this by looking at the strength of the partial correlations of each variable. Are these variables (and the relationships among the variables) explained by underlying factors? Let's see.

We are hoping for an overall MSA of .90 or higher.
```{r}
#KMO test
KMO(correl[,1:23])
```

The MSA = .93. Great! We have solid sampling adequacy to perform an EFA.
...
2. Now I can take a look at the histograms of each item of the data. I should have 23 lines of code, one for each question, and I will set breaks as 5 since they are all on a 5-point Likert scale
```{r}
...
Let's try the code that will show all the histograms in one plot
```{r}
...
I got an error here in the markdown file with the default code, but once I adjusted the margins with the mar = # syntax, I could get it to work. However, only if the margins were set pretty small. Because of this, the question# is cut off from each. However, I also put this code in a blank script and got the plot to show up in my plots pane, and I saved that plot. I will include that too since it's bigger/easier to understand. I could have potentially messed with R Commander to get it to show up even bigger and cleaner, but the one from the plots will do for now, and I will embed it below when I knit the markdown file.

Insights from the histograms are:
Nothing shows up with any 9 responses, further confirming that there is no missing data that is just mislabeled as a 9. 

I need to bear in mind that 1 corresponds to strongly agree. This will be noteworthy as I look more at the item and scale analyses. (1 is high, 5 is low, and not the inverse).
...
There does seem to be some relationships among the variables and nothing looks too weird or out of place. Items are correlated with each other (which is good), but there may be some items that are too correlated with each other (which could be bad), but no major red flags from this so far - mostly just things to keep an eye out for. Another potential issue to stay mindful of is that there are quite a few items that seem to have similar response patterns. I could have some cross-loadings happen with this high number of items that seem to be behaving in similar fashions, but I will know more when I cross that bridge. So far, so good.
...
#relocating the ID variable to the first column
data <- data %>%
    dplyr::select(ID, everything())

#checking my column names, making sure ID is the first one
colnames(data)
```
...
3. I will split the data as 70/30. It is usually recommended to have a larger sample for the EFA (training), so I have chosen this uneven split that favors a bigger sample for the 'test' set. Also, since we practiced the 50/50 split in class, I wanted to try a different one that would more likely be applicable to best practices in the 'real world'
```{r}
...
4. When selecting the method for my EFA, I opted for Maximum Likelihood (ML). ML is good for large datasets, it is well-suited when the assumptions for EFA have been met (as I checked above), and it can handle unequal loadings within factors. I opted to not use Least Squares because it's more commonly used when there is no factor correlation and no large cross-loadings (both of which may occur based on my views of the histograms). I also opted not to use Principla Axis (PA) because it can often underestimate factor loadings and may not provide an accurate view of the underlying factors. ML does operate best when assumptions of normality are met. My hope is that the data is not so highly skewed that ML will inaccurate. If I run an initial EFA that seems to create relationships with variables that I don't think are logical, I may revisit this decision and see what PA will provide for me. However, ML is the most commonly used method, especially in large sets of data.

Here's information on the naming as we move forward:

* fa = Factor Analysis
* ml = Maximum Likelihood (the method of factor analysis we are using)
* 4 = the number of factors we think are in the data (we will change this as we adjust the number of factors)
...
5. I opted for an oblique rotation because I saw quite a bit of correlation between several items, so I am operating under the idea that my factors will be likely be more correlated (if there was evidence they would be less correlated, I could opt for an orthogonal rotation). Per the recommendation of James Gaskin's EFA video, I chose a promax rotation to maximize interpretability.
```{r}
...
Our fit indices are improved, and we're down to just one cross-loading, but one factor only has 2 variables, one has 3 (right at the minimum requires number). I'm going to try principal axis and see if that looks any better
```{r}
...
Once more, we have a factor with only two variables and one with three. I'm leaning toward a 4-factor model because of this, but I will still take a closer look at the four options I have run so far (4-factor ML, 5-factor ML, 4-factor PA, 5-factor PA). I may also remove some items if they seem to be cross-loading, acting differently, or if the question is too different or too redundant compared to the others.

So now, I will input these four into one big excel file to better examine the pros and cons of each.
```{r}
...
The examinations of the four potential models were very useful to determine how to proceed. The first note was that the difference between ML and PA was very slight, so I chose to proceed with ML. Also, the fit indices only improved slightly when moving from the 4-factor to the 5-factor model. The major issue with the five-factor model is the factor that only has two items - this does not meet the standard of 3 items per factor. With this in mind, the ML method 4-factor model is likely what I will proceed with.

The examinations of this model also pointed to some potentially problematic items. Namely:
Q3 had cross-loadings on factors 3 and 4; additionally, it was the only negative loading in the entire model (sometimes the reverse scored items will act weirdly and be more challenging to interpret, so this may be why it is cross-loading). The loadings were also on the lower end (-0.498 and 0.329)
...
Things are looking good! There is no cross-loading, there is no item missing a >.30 loading, and each factor still has at least 3 items. This is our simple structure.
...
6. My final simple structure has 4 factors. Here are the names for each factor (and their alternate names!) followed by their corresponding items:
...
Let's chat about the fit statistics for the final model. As a reminder, the final model is the 4-factor model, ML method, with the three items removed.

7. The Tucker Lewis Index ranges from 0 to 1, and it provides information about the goodness of fit for a factor model. For the final model, the TLI = 0.927. The preferred standard for a good TLI is 0.95 or greater. However, a value of 0.90 or greater may be acceptable as well. It is noteworthy that the obtained value does not reach the preferred standard, but it is above 0.90 so it is considered acceptable. Although the TLI was higher in the 5 factor model, the results were problematic (they were not as easily interpretable, and one factor would not meet the criteria for the minimum number of items, only having two items loading high enough on that factor instead of the standard of 3 or more).

8. The RMSEA Index is another method of evaluating the godness of fit for a factor model. It ranges from 0 to infinity, with lower numbers indicating better fit. For the final model, the RMSEA = 0.049, 90%CI [0.045,0.053]. The preferred standard is less than 0.08. Some standards of practice even call for the RMSEA to be 0.5 or below. The good news is that our obtained RMSEA is lower than both.
...
All four are on scored a scale of 1 (Strongly agree) to 5 (Strongly disagree) (although honestly, it may make more sense to invert the scale in the future so strongly disagree is associated with 1 instead).
...
Note that the first set of numbers is the Question_#, and the second set of numbers is the new column number due to the removal of three items.
...
The last step is some scale analysis, evaluating the reliability of each subscale by calculating Cronbach's alpha.
...
Now I can run a quick check for any items that need reverse-scoring. I feel like I know the items quite well at this point, but I can double-check with the skimr package.
```{r}
library(skimr)
...
I'm mostly noting these so if something suspicious crops up in my reliability analysis, I have a good idea of where to start in the troubleshooting process.
...
Here are some notes for interpreting the above results:
...
Once more, here's so notable information from above:
Scale Info - 
* raw_alpha (0.82)
* std.alpha (0.82)
* G6(smc) (0.76)
...
The next steps would be more involved and require extensive work and re-analyzing of the data.
...
In summation regarding the reliability analyses for all four subscales, the FITC scale meets the adequate standard for reliability (0.79), and the FITM and FITS scales both meet the higher 'good' standard for reliability (0.82 and 0.80, respectively). The reliability results for the FITC, FITM, and FITS did not flag any items for potential removal.
The 'problem child' is the SFTS scale; it did not reach the minimum acceptable standard for reliability (0.59), and the results did not flag any items for potential removal. From the potential options listed above regarding strategies for increasing the reliability of SFTS, the two recommended paths forward would be:
- Rerun the EFA using a three factor model, potentially removing additional items to ensure each factor captures a clear and delineated construct.
